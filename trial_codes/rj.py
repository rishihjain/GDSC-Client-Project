...
#                    f"Finally the description for each day which should look like if a human is speaking. Write each day itinerarary in the form of paragrah under the heading of each day in the newline."\
#                    f" Also add distance of travel for each day and approx time in the same paragraph(do not give null values if not available) of travel. "\
#  \
...
#                    f"for {input_dict['num_tourists']} mainly with their {input_dict['type_of_travelers']} with an average age of {input_dict['average_age']}.The " \
#                    f"primary interests for activities are {input_dict['genre']}.The preferred mode(s) of travel include " \
#                    f"{input_dict['mode_of_travel']}.The group prefers {input_dict['food']} food. Please write the detailed itinerary as a human in speaking  " \
#                    f"plan for each day . Also write a title for each day in the fist line and then the itinerary for that day in a paragraph. Also consider the factors like activities, locations, weather according to the season they are " \
#                    f"travelling. " \
#                    f"Ensure to consider the preferences and " \
#                    f"interests of the group for each day's schedule. Important considerations: Factor in travel time " \
#                    f"between destinations. Suggest local transportation options. Include a mix of activities that cater" \
#                    f" to the group's interests." \
#                    f"of travel. " \
#                    f"Give a title to the Complete itinerary but make sure you don't repeat location names in multiple days also you can mention prime locations in title that are going to be there in iternary. Do not suggest any activities " \
#                    f"in the first city if the travel time and distance from the source city is more otherwise we can suggest activities." \
#                    f"If required you can also give a NOTE to the travellers at the end of each day regarding the important considerations required for that day(But it is not compulsory for each day). "\
#  \
